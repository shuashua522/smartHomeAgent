...
# ===================== 1. 定义核心函数（合并所有场景为all计算 + 保留Dunn检验修复） =====================
...
def one_way_anova_for_all_scenes(df, factor="系统", target_scenes=["A", "B", "C", "D"],
                                 metrics=["输入负担", "可用性", "接受度"]):
    """
    合并指定场景（A/B/C/D）的数据为all场景，做单因素方差分析（保留Dunn检验修复逻辑）
    核心修改：不再遍历单个场景，合并所有目标场景为all场景统一计算
    """
    all_results = []
    # 定义总场景名称
    total_scene = "all"
    print(f"\n==================== 场景{total_scene} 分析结果（合并A/B/C/D） ====================")

    # 核心修改1：合并A/B/C/D所有场景的数据作为all场景
    df_all = df[df["场景"].isin(target_scenes)].copy()
    if len(df_all) == 0:
        print(f"场景{total_scene} 无数据（A/B/C/D均无数据），跳过")
        all_results.append({"场景": total_scene, "指标": "无", "状态": "跳过", "原因": "合并后无数据"})
        return pd.DataFrame(all_results)

    sys_count = df_all[factor].nunique()
    if sys_count < 2:
        print(f"场景{total_scene} 只有{sys_count}个系统数据，无法分析，跳过")
        all_results.append({"场景": total_scene, "指标": "无", "状态": "跳过", "原因": "有效系统数不足"})
        return pd.DataFrame(all_results)
    ...
    for metric in metrics:
        ...
        try:
            ...
            groups = []
            sys_names = []
            for sys in df_all[factor].unique():
                sys_data = df_all[df_all[factor] == sys][metric].dropna()
                if len(sys_data) >= 2:
            ...
            if len(groups) < 2:
                ...
                all_results.append({"场景": total_scene, "指标": metric, "状态": "跳过", "原因": "有效系统数不足"})
                continue
            ...
            all_results.append({
                "场景": total_scene, "指标": metric, "状态": "失败", "原因": error_info
            })
    return pd.DataFrame(all_results)
...
if __name__ == "__main__":
    ...
    # 核心修改2：调用合并场景的函数，指定要合并的源场景
    results_df = one_way_anova_for_all_scenes(
        df=df,
        target_scenes=["A", "B", "C", "D"],  # 要合并的源场景
        metrics=["输入负担", "可用性", "接受度"]
    )

    # 核心修改3：输出文件名为all场景对应名称
    results_df.to_excel("all场景系统方差分析结果_最终版.xlsx", index=False)
