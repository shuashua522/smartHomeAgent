...
def one_way_anova_by_scenario(df, factor="系统", scenarios=["A", "B", "C", "D"],
    ...
    """
    按场景做单因素方差分析（自动适配：正态数据用参数检验，非正态数据用非参数检验）
    核心修改：
    1. 正态性全满足 → 标准ANOVA/Welch ANOVA + Tukey HSD
    2. 正态性不满足 → Kruskal-Wallis非参数检验 + Dunn事后检验
    3. 所有指标按"得分越高越好"排名
    """
    all_results = []
    for scene in scenarios:
        ...
        for metric in metrics:
            ...
            try:
                ...
                if normality_pass:  # 所有组都符合正态性 → 参数检验
                ...
                else:  # 存在非正态组 → 非参数检验（Kruskal-Wallis H检验）
                ...
                if result_flag:
                    ...
                    # 5. 事后检验：自动选择Tukey HSD（参数）/Dunn检验（非参数）
                    ...
                    if normality_pass:
                        ...
                        # 非参数事后检验：Dunn检验（Bonferroni校正）
                        # 准备数据
                        posthoc_data = np.concatenate([np.array(g) for g in groups])
                        posthoc_groups = np.concatenate([[sys] * len(g) for sys, g in zip(sys_names, groups)])
                        # 执行Dunn检验
                        dunn_result = sp.posthoc_dunn(
                            data=pd.DataFrame({
                                'value': posthoc_data,
                                'group': posthoc_groups
                            }),
                            val_col='value',
                            group_col='group',
                            p_adjust='bonferroni'
...
if __name__ == "__main__":
    # ---------------- 请修改这里的CSV文件路径 ----------------
    CSV_FILE_PATH = "data.csv"  # 替换为你的CSV文件实际路径（如：F:/data.csv）
    # ---------------------------------------------------------
    ...
    # 步骤3：保存结果到Excel（新增检验类型、正态性等关键信息）
